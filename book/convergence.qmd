...
### 3.1.1 The Changeless-Ground Dilemma {#sec-3-1-10}
...
*Dependency.* [§1.3](groundwork.html#sec-1-3) alone. No S5, no ∅-floor, and no converse from ◇C → (T∧S∧Φ). *Structure:* a no-onset reductio. *Scope:* it rules out emergence from capacitylessness; it does not independently derive □◇C. The exact cross-world burden is recorded at [§3.5.4](convergence.html#sec-3-5-4).
...
**Doubles as the section's backstop:** a state with no T, S, Φ is no state at all, the ∅-incoherence every other Move leans on, developed in its own right at [§3.2.6](convergence.html#sec-3-2-6).
...
1. And if those relations are *borne* rather than a bare Humean mosaic of labels, something sustains them (Φ), the step the bare mosaic contests, which this Move inherits rather than earns, the anti-Humean Φ being earned downstream at [§2.4.8](the-argument.html#sec-2-4-8) (the causal-exchange derivation) and [§4.1.4](metaphysical-rivals.html#sec-4-1-4) (the Humean reading).
...
*Dependency.* [§1.3](groundwork.html#sec-1-3) and [§1.4](groundwork.html#sec-1-4) supply the ordinary triad; Chapter 2 supplies the formal T/S/Φ notation. The section handles primitive thisness (Adams) and mereological nihilism (van Inwagen, Dorr); the anti-Humean Φ the bare mosaic (Lewis) denies is earned at [§4.1.4](metaphysical-rivals.html#sec-4-1-4), on the causal-exchange derivation of Φ at [§2.4.8](the-argument.html#sec-2-4-8). It defends the conclusion against a counterexample rather than proving it single-handed.
...
1. The lift from "operative in this case" to "necessary" is not made here: it is borrowed from [§3.3.4](convergence.html#sec-3-3-4) (conceivability), the modal triconditional, or the ∅-result ([§3.2.6](convergence.html#sec-3-2-6)).
...
1. That item would be real yet stand in no directed order, bear no determinate difference, and have no bearing or realization; by step 2 there is no such roleless standing for it to take.

1. So nothing real lies outside Nature, and nothing of Nature lies outside Reality.
...
1. Let G be a proposed ground additional to Nature. Relative to Nature, G must either (i) ground Nature, (ii) be grounded by Nature, or (iii) stand in no grounding relation to Nature. Grounding is asymmetric, so the three positions exhaust the available directions ([§1.5.5](groundwork.html#sec-1-5-5)).

1. If G grounds Nature, the grounding relation must be directed from ground to grounded, distinguish G from what it grounds, and obtain as a real dependence rather than a verbal pairing. At role-resolution, those are T, S, and Φ: directed order, difference, and bearer. The proposed external ground therefore instantiates the structure it was meant to precede ([§2.2.2](the-argument.html#sec-2-2-2)).
...
**Stage 2, □(T∧S∧Φ) at role-resolution (§[§3.2](convergence.html#sec-3-2)–3.3):** convergent Moves across construction, determinacy, modal vocabulary, enactment, and the null-state floor reach necessary direction, difference, and bearer without S5. These lines are not premise-disjoint; several share the ∅-floor and the constitutive role-identifications. Their force is nevertheless plural: facthood, world-determinacy, modal content, performative indispensability, and null-state incoherence expose different failure points. The role-level conclusion survives any one local cut, while the shared joints remain explicit.
...
The role-level necessity of direction, difference, and bearer is the chapter's strongest convergence. Several lines lean on the ∅-floor, but that floor is defended from facthood, from the act/object closure on conceiving nothing, and independently in Chapter 2. Several lines also depend on the constitutive identification of obtaining, difference, and bearing with the three roles. That identification is a shared premise and is named as one; multiplying its applications does not multiply its warrant.
...
Each limb still owes the step from its generic role to its specific type: directed ordering to succession (T), distinctness to co-existence (S), persistence to a causal-persistent being (Φ). All three residuals close on □◇C, the spatial and persistence steps each leaning additionally on a Stage-2 realizer biconditional.
...
1. Stage 2 re-establishes, without S5, that every possible world instantiates direction, difference, and bearer: □(T∧S∧Φ), at role-resolution (§[§3.2](convergence.html#sec-3-2)–3.3).
...
1. It does not yet do so. Stage 2 directly earns directed order, difference, and bearer. The upgrades to temporal succession, spatial extension, and causal persistence are carried at [§3.5.3](convergence.html#sec-3-5-3), and those upgrades currently use □◇C.
