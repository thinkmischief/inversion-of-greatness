...
The impossibility that matters most for the argument is a specific one: **∅**, the proposed total absence of the structural roles. It counts as impossible not because it is unlikely but because it is precluded — [§2.8](the-argument.html#sec-2-8) argues that no possible world can realize it, the result declared as the *Exclusivity of Nature* ([§2.8.6](the-argument.html#sec-2-8-6)).

**The modal fallacy.** The most common beginner error is the slide from *actual* to *necessary*: from "this is the case" to "this had to be the case." The configuration of atoms on this page is actual and contingent — it could have been otherwise. That each atom is identical to itself is necessary. Confusing the two flattens contingency into necessity, and it is a standing hazard in modal reasoning. (It is distinct from the *modal-collapse* worry of [§4.4.9](metaphysical-rivals.html#sec-4-4-9), which presses the converse: that S5 over-tightens necessity into actuality.)
...
| System | Adds | Schema | Frame condition | Reads / why you'd want it |
| --- | --- | --- | --- | --- |
| K |  —  | □(P → Q) → (□P → □Q) | none | necessity distributes over implication — the bare minimum any modal logic needs |
| T | T | □P → P | reflexive | what is necessary is actually the case — drop this and "necessary" no longer implies "true" |
| S4 | 4 | □P → □□P | transitive | what is necessary is necessarily necessary — necessity doesn't itself vary |
| B | B | P → □◇P | symmetric | what is actual is necessarily possible — no world is cut off from ones that can reach it |
| S5 | 5 | ◇P → □◇P | equivalence | what is possible is necessarily possible — modal status is the same from every world |

The main chain runs K ⊂ T ⊂ S4 ⊂ S5, each adding its constraint to the last; B sits off the chain but supplies the axiom S5 also validates. The book uses **S5**, where accessibility is a full equivalence relation (reflexive, transitive, symmetric): relative to any world, modal status is invariant. One small consequence is worth naming: the T-axiom (□P → P) together with the P → ◇P principle of [§1.2](groundwork.html#sec-1-2) yields **Necessity entails Possibility**, □P → ◇P — stated at [§2.1.4](the-argument.html#sec-2-1-4) and used at [§6.1](relocating-greatness.html#sec-6-1) to block necessity-based rescues of the supernatural. *Why* S5 rather than a weaker system is Part 2's second debate — and Part 3 is where this book's answer differs from the usual one.
...
**The book's modality at work: a ladder of shapes.** The vocabulary can be walked through one domain — shapes — without leaning on a plurality of worlds. A *round square* is **impossible**, ruled out not by logical form but by what extension is: no region can bear two incompatible boundary-determinations at once ([§2.4.7](the-argument.html#sec-2-4-7)). A *square* or a *triangle* is **possible**, each a coherent structure, ◇. That *squared or triangled things exist* is **possibly instantiated** — the existence-committing claim, distinct from the bare concept. And what all of this presupposes — that there is extension at all, with distinct positions to bound — climbs toward the **necessary structure** the argument isolates as the Structural Requirement ([§2.4](the-argument.html#sec-2-4)).

The ladder is an intuition pump, not a proof: that a round square is impossible already presupposes the structure of space, so the sequence *illustrates* the book's modality and hands the derivation off to [§2.1](the-argument.html#sec-2-1), where S5 is earned rather than assumed.
...
Spelled out: Temporality, Spatiality, and Physicality (the structural Conditions of [§1.4](groundwork.html#sec-1-4)) are necessary — not "happen to be instantiated," but necessary, full stop. This is Theorem 6 (*Necessity of Nature*), declared at [§2.8.5](the-argument.html#sec-2-8-5). *Nature* is defined at [§1.4.4](groundwork.html#sec-1-4-4): N =df T∧S∧Φ.
...
**The groundwork.** The lift from possibility to necessary possibility is not imported as an S5 axiom. Modal Bivalence (∀P: ◇P ∨ ¬◇P, [§2.1.1](the-argument.html#sec-2-1-1)) is paired with the fixed global real-possibility domain W: because ◇P means truth at some member of W, and W is the same domain at every world, ◇P → □◇P follows (T1, [§2.1.10](the-argument.html#sec-2-1-10)). The chain takes that as established.

::: {.argument}
**Necessity of Nature** · *The Cogito–S5 Pipeline*

1. *Necessary Possibility.* Whatever is possible is necessarily possible. ◇P → □◇P. (T1, [§2.1.10](the-argument.html#sec-2-1-10))

1. By universal instantiation, taking change for P. ◇C → □◇C.

1. *Cartesian Certainty.* Change is really possible. ◇C. ([§2.2.1](the-argument.html#sec-2-2-1))

1. By modus ponens on 2 and 3, the possibility of change is itself necessary. □◇C. (Lemma 3, [§2.3](the-argument.html#sec-2-3))

1. *Structural Requirement.* Necessarily, change is not so much as possible unless T, S, and Φ are instantiated. □(◇C → (T∧S∧Φ)). ([§2.4](the-argument.html#sec-2-4))

1. By the K-axiom, distributing the box over the conditional. □◇C → □(T∧S∧Φ).

1. By modus ponens on 4 and 6. □(T∧S∧Φ). (Triconditional Necessity, [§2.5](the-argument.html#sec-2-5); Necessity of Nature, T6, [§2.8.5](the-argument.html#sec-2-8-5))

:::

**Where the load falls.** Modal Bivalence and Modal Constitution earn the Necessary Possibility theorem — deny either and the chain never starts ([§2.1](the-argument.html#sec-2-1)). Cartesian Certainty (line 3) is the only contact with actuality: reading this sentence is a change, and doubting that change occurs is itself a change, so the input cannot be coherently denied (*Cogito Ergo Muto*, [§2.2.1](the-argument.html#sec-2-2-1)). The Structural Requirement (line 5) is the load-bearing identification — the place a careful reader is most likely to stall — and §[§2.4](the-argument.html#sec-2-4)–2.6 are dedicated to it.

**Reading the result.** The derivation does not say any *particular* thing must change. It says T, S, and Φ are instantiated in every possible arrangement of things: what is necessary is the structural Condition-set, not the contingent contents. Its companion result closes the door the other way: that the total absence ∅ cannot obtain is the *Exclusivity of Nature* (Theorem 7, [§2.8.6](the-argument.html#sec-2-8-6)).
