...
*Why the null state is impossible.* The Chapter-2 line reached this verdict (¬◇∅) through heavy modal machinery, its own Null-State reductio. Here it rests on something lighter and earlier: the Chapter-1 result that *something is the case* (§3.2.1). Because that line needs no S5, the backstop is the most secure floor in the chapter, the one every other Move can fall back to. It has a second, independent support as well: to so much as assert that *nothing* is the case is itself something being the case, so the denial refutes itself (§3.2.1).
